...
async function accumulateParentBlockHash(currentBlockNumber, parentHash) {
    ...
    const historyAddress = util_1.Address.fromString((0, util_1.bigIntToHex)(this.common.param('vm', 'historyStorageAddress')));
    ...
    if ((await this.stateManager.getAccount(historyAddress)) === undefined) {
        await this.evm.journal.putAccount(historyAddress, new util_1.Account());
    }
    ...
    if (parentBlock.header.timestamp < forkTime) {
        ...
        for (let i = 0; i < Number(historyServeWindow) - 1; i++) {
            if (ancestor.header.number === util_1.BIGINT_0) {
                break;
            }
